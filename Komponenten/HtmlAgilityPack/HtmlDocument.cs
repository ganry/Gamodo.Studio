...
namespace HtmlAgilityPack
{
    ...
    public partial class HtmlDocument
    {
        ...
        public List<PHPCode> PHPCodeList = new List<PHPCode>();
        ...
        private bool NewCheck()
        {
            ...
            if (_index < Text.Length)
            {
                ...
                {
                    phpCode = new PHPCode();
                    phpCode.BeginPos = _index - 1;

                    _oldstate = _state;
                    _state = ParseState.PHPTag;
                    //PushNodeStart(HtmlNodeType.Text, _index - 1);
                    return true;
                }
            }
                    ...
                    case ParseState.PHPTag:
                        if (_c == '?')
                        {
                            if (_index < Text.Length)
                            {
                                if (Text[_index] == '>')
                                {
                                    phpCode.EndPos = _index;
                                    phpCode.Text = Text.Substring(phpCode.BeginPos + 2, phpCode.EndPos - (phpCode.BeginPos +3));
                                    PHPCodeList.Add(phpCode);
