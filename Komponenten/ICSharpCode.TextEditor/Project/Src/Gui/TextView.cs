...
//			bgColorBrush.Dispose();
			...
			foreach (MarkerToDraw m in markersToDraw) {
				...
				switch (marker.TextMarkerType) {
					...
					case TextMarkerType.SolidBlock:
						...
						break;
                    case TextMarkerType.QuickMark:
                        g.FillRectangle(BrushRegistry.GetBrush(marker.Color), drawingRect.X, drawYPos-2, drawingRect.Width, 3);
                        //g.DrawLine(BrushRegistry.GetPen(marker.Color), drawingRect.X, drawYPos +3, drawingRect.Right, drawYPos + 3); 
                        break;
                    case TextMarkerType.CopyMarker:
                        g.DrawRectangle(BrushRegistry.GetDotPen(marker.Color), Rectangle.Round(drawingRect));
                        //g.DrawLine(BrushRegistry.GetPen(marker.Color), drawingRect.X, drawYPos +3, drawingRect.Right, drawYPos + 3); 
                        break;
				}
			}
		...
		{
            g.FillRectangle(BrushRegistry.GetBrush(Color.FromArgb(50, 97, 174, 255)), rect);
            g.DrawRectangle(new Pen(Color.FromArgb(255, 51, 153, 255)), rect);
		}
