...
namespace ICSharpCode.TextEditor.Gui.CompletionWindow
{
	public class CodeCompletionWindow : AbstractCompletionWindow
	{
		...
		public static CodeCompletionWindow ShowCompletionWindow(Form parent, TextEditorControl control, string fileName, ICompletionDataProvider completionDataProvider, char firstChar, string optional = "")
		{
			return ShowCompletionWindow(parent, control, fileName, completionDataProvider, firstChar, optional, true, true);
		}
		
		public static CodeCompletionWindow ShowCompletionWindow(Form parent, TextEditorControl control, string fileName, ICompletionDataProvider completionDataProvider, char firstChar, string optional, bool showDeclarationWindow, bool fixedListViewWidth)
		{
			ICompletionData[] completionData = completionDataProvider.GenerateCompletionData(fileName, control.ActiveTextAreaControl.TextArea, firstChar, optional);
		...
		CodeCompletionWindow(ICompletionDataProvider completionDataProvider, ICompletionData[] completionData, Form parentForm, TextEditorControl control, bool showDeclarationWindow, bool fixedListViewWidth) : base(parentForm, control)
		{
            ...
            this.fixedListViewWidth = false;
            this.areaControl = control.ActiveTextAreaControl;
            areaControl.DoHandleMousewheel = false;
            this.FormClosed += new FormClosedEventHandler(CodeCompletionWindow_FormClosed);
        ...
        void CodeCompletionWindow_FormClosed(object sender, FormClosedEventArgs e)
        {
            areaControl.DoHandleMousewheel = true;
        }
		...
		Size GetListViewSize()
		{
			...
			if (!fixedListViewWidth) {
				width = GetListViewWidth(width+50, height);
			}
