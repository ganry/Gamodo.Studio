...
namespace ICSharpCode.TextEditor.Document
{
	internal sealed class LineManager
	{
		...
		public void Replace(int offset, int length, string text, bool editLineState = true)
		{
			...
			if (!string.IsNullOrEmpty(text)) {
                InsertInternal(offset, text, editLineState);
			}
//			#if DEBUG
...
//			#endif
        ...
        void SetSegmentLength(LineSegment segment, int newTotalLength, bool editLineState = true)
		{
			int delta = newTotalLength - segment.TotalLength;
			if (delta != 0) {
				lineCollection.SetSegmentLength(segment, newTotalLength);
				OnLineLengthChanged(new LineLengthChangeEventArgs(document, segment, delta, editLineState));
			}
		}
...
//			Debug.Assert(logicalLineNumber >= foldEnd);
		...
		{
            if (e.EditLineState)
                e.LineSegment.State = LineState.Modified;
