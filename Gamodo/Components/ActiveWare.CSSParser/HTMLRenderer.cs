		...
		static string Render(SimpleSelector ss) {
			...
			if (ss.Combinator.HasValue) {
				switch (ss.Combinator.Value) {
					case ActiveWare.CSS.Combinator.PrecededImmediatelyBy: txt.Append(" <span class=\"combinator\">+</span> "); break;
					case ActiveWare.CSS.Combinator.ChildOf: txt.Append(" <span class=\"combinator\">&gt;</span> "); break;
					case ActiveWare.CSS.Combinator.PrecededBy: txt.Append(" <span class=\"combinator\">~</span> "); break;
				}
			}
		...
		static string Render(Term t) {
			...
			if (t.Type == TermType.Function) {
				...
				if (t.Unit.HasValue) {
					...
					if (t.Unit.Value == ActiveWare.CSS.Unit.Percent) {
						txt.Append("%");
					} else {
